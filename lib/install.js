...
export function getOpenCodeConfigPaths(configDir = getDefaultOpenCodeConfigDir()) {
  return {
    ...
    commandDir: join(configDir, "command"),
...
function removeHarnessCommands(commands) {
  ...
  for (const name of Object.keys(next)) {
    if (name.startsWith("openspec-harness:")) {
      delete next[name];
    }
  }
  return next;
...
function mergeOpenCodeConfig(paths, packageName, sourceConfig, { pluginMode, packageSpec, commandMode, pruneConfigCommands }) {
  ...
  if (commandMode === "config") {
    next.command ??= {};
    Object.assign(next.command, sourceConfig.command ?? {});
  } else if (pruneConfigCommands) {
    next.command = removeHarnessCommands(next.command);
    if (Object.keys(next.command).length === 0) {
      delete next.command;
    }
  }
...
function syncCommandFiles(root, paths, sourceConfig) {
  mkdirSync(paths.commandDir, { recursive: true });
  for (const entry of readdirSync(paths.commandDir, { withFileTypes: true })) {
    if (entry.isFile() && entry.name.startsWith("openspec-harness-") && entry.name.endsWith(".md")) {
      rmSync(join(paths.commandDir, entry.name), { force: true });
    }
  }

  const sourceCommandDir = join(root, ".opencode", "command");
  if (existsSync(sourceCommandDir)) {
    cpSync(sourceCommandDir, paths.commandDir, { recursive: true });
  }

  return Object.keys(sourceConfig.command ?? {}).length;
}
...
export function installOpenSpecHarness({
  ...
  if (syncCommands) mkdirSync(paths.commandDir, { recursive: true });
  ...
  const commandFileCount = syncCommands && commandMode === "files"
    ? syncCommandFiles(root, paths, sourceConfig)
    : 0;
...
export function doctorOpenSpecHarness({
  ...
  const configCommands = Object.keys(config.command ?? {});
  const commandFiles = existsSync(paths.commandDir)
    ? readdirSync(paths.commandDir).filter((entry) => entry.startsWith("openspec-harness-") && entry.endsWith(".md"))
    : [];
  const skillsPresent = existsSync(join(paths.skillsDir, "openspec-harness-explore", "SKILL.md"));
  const commandFilesPresent = commandFiles.includes("openspec-harness-explore.md");
  ...
  if (!commandFilesPresent && !configCommands.includes("openspec-harness:explore")) findings.push({ severity: "error", message: "OpenSpec Harness commands are missing from command files or OpenCode config." });
  ...
  return {
    ...
    configCommandCount: configCommands.filter((command) => command.startsWith("openspec-harness:")).length,
    commandCount: commandFiles.length + configCommands.filter((command) => command.startsWith("openspec-harness:")).length,
    opencode,
    findings
  };
}
