...
export async function discoverChanges(cwd) {
  const changesRoot = join(cwd, "openspec", "changes");
  const directories = await listDirectories(changesRoot);
  const activeChanges = [];

  for (const directory of directories) {
    if (directory === "archive" || directory.startsWith(".")) {
      continue;
    }
    ...
    if (await pathExists(join(changesRoot, directory, "proposal.md"))) {
      activeChanges.push(directory);
    }
  }
