...
async function readJsonOrJsonc(path) {
  const text = await readFile(path, "utf8");
  return JSON.parse(path.endsWith(".jsonc") ? stripJsonComments(text) : text);
}
...
async function readOpenCodeConfig(cwd) {
  ...
  if (await pathExists(jsoncPath)) {
    return {
      path: jsoncPath,
      config: await readJsonOrJsonc(jsoncPath)
    };
  }
  return {
    path: jsonPath,
...
async function commandFileNames(cwd) {
  const commandRoot = join(cwd, ".opencode", "command");
  if (!(await pathExists(commandRoot))) return [];
...
function expectedSkillForCommand(command, entry = null) {
  if (command.startsWith("openspec-harness-")) return command;
...
export async function syncDocs(cwd) {
  ...
  const { path: opencodePath, config } = await readOpenCodeConfig(cwd);
  ...
  const configCommands = commandNames(config);
  const fileCommands = await commandFileNames(cwd);
  const commands = [...new Set([...configCommands, ...fileCommands])].sort();
  const expectedSkills = configCommands
    .map((command) => expectedSkillForCommand(command, config.command?.[command]))
    .filter(Boolean)
    .concat(fileCommands.map((command) => expectedSkillForCommand(command)).filter(Boolean));
  const findings = [];
  ...
  for (const skill of skills) {
    const colonCommand = `openspec-harness:${skill.replace(/^openspec-harness-/u, "")}`;
    const fileCommand = skill;
    if (!commands.includes(colonCommand) && !commands.includes(fileCommand)) {
      findings.push({
        severity: "warning",
        area: "commands",
        message: `Skill ${skill} has no matching user command ${colonCommand} or ${fileCommand}.`
      });
    }
  }
  ...
  for (const name of ["context-sync", "docs-sync"]) {
    if (
      (commands.includes(`openspec-harness:${name}`) || commands.includes(`openspec-harness-${name}`)) &&
      !readme?.includes(`openspec-harness:${name}`) &&
      !readme?.includes(`openspec-harness-${name}`)
    ) {
      findings.push({
        severity: "warning",
